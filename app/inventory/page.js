...
function getItemsForLocation(loc) {
  return loc === "Central Store"
    ? getCentralStoreAsUnitItems()
    : getUnitInventory(loc);
}
...
export default function InventoryPage() {
  ...
  const allLocationOptions = useMemo(() => {
    ...
    LOCATIONS.forEach((loc) => {
      getItemsForLocation(loc).forEach((item) =>
        list.push({ name: item.name, location: loc }),
      );
    });
    return list;
  }, []);
  ...
  const stockPct = item
    ? Math.max(
        0,
        Math.min(100, Math.round((item.stock / (item.reorderPoint * 2)) * 100)),
      )
    : 0;
  const forecast = useMemo(
    () =>
      item ? computeForecast(item, { autoReorder: aiReorderEnabled }) : null,
    [item, aiReorderEnabled],
  );
  const predictionChartData = forecast
    ? [
        ...forecast.history.map((v, i) => ({
          month: forecast.historyMonthLabels[i],
          actual: v,
          predicted: null,
        })),
        {
          month: forecast.currentMonthLabel,
          actual: forecast.history[forecast.history.length - 1] ?? null,
          predicted: forecast.history[forecast.history.length - 1] ?? null,
        },
        ...forecast.predicted.map((v, i) => ({
          month: forecast.predictedMonthLabels[i],
          actual: null,
          predicted: v,
        })),
      ]
  ...
  const sortedExpiry = useMemo(
    () => [...EXPIRY_ITEMS].sort((a, b) => a.daysToExpiry - b.daysToExpiry),
    [],
  );
  const sortedVariance = useMemo(
    () =>
      [...STOCK_VARIANCE].sort(
        (a, b) => Math.abs(varianceOf(b)) - Math.abs(varianceOf(a)),
      ),
    [],
  );
  const filteredInventory = useMemo(() => {
    ...
    return unitItems.filter(
      (i) =>
        ...
        computeRisk(i).toLowerCase().includes(q),
    );
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-6 py-10">
            ...
            <div className="absolute z-30 mt-2 w-full glass-strong border border-white/12 rounded-xl p-1.5 max-h-72 overflow-y-auto">
              {globalMatches.length === 0 ? (
                <div className="px-3 py-4 text-center text-[12.5px] text-slate-500">
                  No items match &ldquo;{globalQuery}&rdquo;.
                </div>
              ) : (
                  ...
                  >
                    ...
                    <span
                      className={`shrink-0 text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full border ${
                        m.location === "Central Store"
                          ? "text-ai-violet border-ai-violet/30 bg-ai-violet/10"
                          : "text-ai-cyan border-ai-cyan/30 bg-ai-cyan/10"
                      }`}
                    >
                      {m.location}
                    </span>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <div className="relative">
            ...
            >
              {unit === "Central Store" ? (
                <FiPackage className="text-ai-violet shrink-0" size={15} />
              ) : (
                <FiHome className="text-ai-cyan shrink-0" size={15} />
              )}
              {unit}
              <FiChevronDown
                size={14}
                className={`text-slate-400 transition-transform ${unitMenuOpen ? "rotate-180" : ""}`}
              />
            </button>
              ...
              {unitMenuOpen && (
                <>
                  ...
                  >
                    <button
                      onClick={() => {
                        changeUnit("Central Store");
                        setUnitMenuOpen(false);
                      }}
                      className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm text-left transition-colors ${
                        unit === "Central Store"
                          ? "bg-ai-violet/15 text-ai-violet"
                    ...
                    <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-500 px-3 pt-1 pb-1.5">
                      Hospital Units
                    </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 text-xs font-semibold px-3.5 py-2.5 rounded-xl border transition-colors ${
              aiReorderEnabled
                ? "border-ai-cyan/50 bg-ai-cyan/10 text-ai-cyan glow-cyan"
                : "border-white/12 text-slate-400 hover:text-slate-200 hover:border-white/20"
            }`}
          >
            <FiZap size={13} />{" "}
            {aiReorderEnabled ? "AI Reordering: On" : "Enable AI Reordering"}
          </button>
        </div>
            ...
            <FadeIn>
              <GlowCard className="mb-6 overflow-x-auto">
                <h3 className="text-white font-bold text-sm mb-1 flex items-center gap-2">
                  <FiBarChart2 className="text-ai-violet" /> LMIS Overview —{" "}
                  {unit}
                </h3>
                <p className="text-[11.5px] text-slate-500 mb-4">
                  Average Monthly Consumption (AMC), Reorder Level (ROL),
                  safety/max stock, and months of stock (MOS) remaining, with a
                    ...
                    {unitItems.map((i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2.5 pr-3 text-slate-400 tabular-nums">
                            {i.amc}
                          </td>
                          <td className="py-2.5 pr-3 text-slate-400 tabular-nums">
                            {i.reorderPoint}
                          ...
                          <td className="py-2.5 pr-3">
                            <span
                              className={`px-2 py-0.5 rounded-full text-[11px] font-bold border whitespace-nowrap ${LMIS_STATUS_STYLE[lmis.status]}`}
                            >
                              {lmis.status}
                            </span>
                          </td>
                          <td className="py-2.5 text-slate-400 text-[12px] max-w-xs">
                            {lmis.recommendation}
                          </td>
                        </tr>
                      );
                    ...
                    {unitItems.map((i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td
                            className={`py-2.5 pr-3 tabular-nums ${i.stock < i.reorderPoint ? "text-warn font-semibold" : "text-slate-300"}`}
                          >
                            {i.stock}
                          </td>
                          <td className="py-2.5 pr-3 text-slate-400 tabular-nums">
                          ...
                          <td className="py-2.5">
                            <span
                              className={`px-2 py-0.5 rounded-full text-[11px] font-bold border ${RISK_STYLES[r]}`}
                            >
                              {r}
                            </span>
                          </td>
                ...
                <GlowCard>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="text-white font-bold text-sm">
                      {item.name} — Predicted Demand
                    </h3>
                  </div>
                  <p className="text-[12px] text-slate-500 mb-4">
                    {unit} · Reorder point: {item.reorderPoint} units · trend:{" "}
                    {forecast.trend >= 0 ? "+" : ""}
                    {(forecast.trend * 100).toFixed(1)}%/mo
                  </p>
                  <div className="mb-5">
                    <LevelBar
                      percent={stockPct}
                      color={RISK_BAR_COLOR[itemRisk]}
                  ...
                  {aiReorderEnabled ? (
                    forecast.stockoutMonth ? (
                      <div className="mt-3 p-3 rounded-lg bg-danger/10 border border-danger/25 text-[12.5px] text-danger flex items-center gap-2">
                        <FiAlertTriangle size={14} className="shrink-0" /> Even
                        with AI reordering active, the current lead time
                        can&apos;t keep pace with demand — a stockout is still
                        projected for {forecast.stockoutMonthLabel}. Consider
                        expediting this order or raising the safety stock level.
                      </div>
                      ...
                      <div className="mt-3 p-3 rounded-lg bg-mint/10 border border-mint/25 text-[12.5px] text-mint flex items-center gap-2">
                        <FiCheckCircle size={14} className="shrink-0" /> AI
                        Reordering active — a replenishment order is projected
                        for {forecast.reorderPlacedMonthLabel}, arriving{" "}
                        {forecast.reorderArrivesMonthLabel}, keeping stock above
                        zero throughout.
                      </div>
                    ) : (
                      <div className="mt-3 p-3 rounded-lg bg-mint/10 border border-mint/25 text-[12.5px] text-mint flex items-center gap-2">
                        <FiCheckCircle size={14} className="shrink-0" /> AI
                        Reordering active — stock is projected to stay above the
                        reorder point for the next 6 months, so no order is
                        needed yet.
                      </div>
                    )
                  ) : itemRisk !== "Low" && forecast.stockoutMonth ? (
                    <div className="mt-3 p-3 rounded-lg bg-danger/10 border border-danger/25 text-[12.5px] text-danger flex items-center gap-2">
                      <FiAlertTriangle size={14} className="shrink-0" /> This
                      item is already below a healthy stock level — at the
                      current consumption trend, with no reorder placed, it's
                      projected to run out in {forecast.stockoutMonthLabel}.
                    </div>
                  ) : (
                    <div className="mt-3 p-3 rounded-lg bg-mint/10 border border-mint/25 text-[12.5px] text-mint flex items-center gap-2">
                      <FiCheckCircle size={14} className="shrink-0" /> Stock is
                      currently healthy — no urgent reorder action needed.
                    </div>
                  )}
                  ...
                  {!aiReorderEnabled && itemRisk !== "Low" && (
                    <div className="mt-3 p-3 rounded-lg bg-warn/10 border border-warn/25 text-[12.5px] text-warn flex items-center gap-2">
                      <FiPackage size={14} /> AI Suggestion: reorder{" "}
                      {Math.max(item.reorderPoint * 2 - item.stock, 100)} units
                  ...
                  {liveMode && (
                    <div className="mt-4 pt-4 border-t border-white/10">
                      ...
                      {liveLoading && (
                        <LiveThinking label="Analyzing with Phantom..." />
                      )}
                      {liveError && (
                        <AIErrorNote message={liveError} onRetry={interpret} />
                      )}
                      {liveText && (
                        <FadeIn>
                          <LiveModeNote>
          ...
          <FadeIn>
            <GlowCard className="overflow-x-auto">
              <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
                <FiClock className="text-warn" /> Items Nearest Expiry
              </h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-white/10">
                  ...
                  {sortedExpiry.map((it) => {
                    ...
                    return (
                      <tr key={it.batch} className="border-b border-white/5">
                        ...
                        <td className="py-2.5">
                          <span
                            className={`px-2 py-0.5 rounded-full text-[11px] font-bold border ${EXPIRY_RISK_STYLE[risk]}`}
                          >
                            {risk === "Expired"
                              ? "Expired"
                              : risk === "OK"
                                ? "OK"
                                : `${it.daysToExpiry}d — ${risk}`}
                          </span>
                        </td>
                      </tr>
                    );
        ...
        {tab === "requisitions" && (
          <FadeIn>
            <GlowCard className="overflow-x-auto">
              <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
                <FiClipboard className="text-ai-cyan" /> Requisition Analyzer
              </h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-white/10">
                    ...
                    <th className="pb-2 pr-3">Typical</th>
                  ...
                  {REQUISITIONS.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="border-b border-white/5">
                        ...
                        <td
                          className={`py-2.5 pr-3 tabular-nums ${flag === "Unusual" ? "text-warn font-semibold" : "text-slate-300"}`}
                        >
                          {r.qtyRequested}
                        </td>
                        ...
                        <td className="py-2.5">
                          <span
                            className={`px-2 py-0.5 rounded-full text-[11px] font-bold border ${flag === "Unusual" ? "text-warn bg-warn/10 border-warn/30" : "text-mint bg-mint/10 border-mint/30"}`}
                          >
                            {flag}
                          </span>
                        </td>
              ...
              <p className="text-[11.5px] text-slate-500 mt-4">
                Flags any requisition at 2× or more, or ¼ or less, of the
                department&apos;s typical order quantity.
              </p>
            </GlowCard>
          </FadeIn>
        )}
        ...
        {tab === "variance" && (
          <FadeIn>
            <GlowCard className="overflow-x-auto">
              <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
                <FiActivity className="text-ai-violet" /> Stock Variance
                Dashboard
              </h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-white/10">
                    <th className="pb-2 pr-3">Item</th>
                    <th className="pb-2 pr-3">Physical</th>
                    <th className="pb-2 pr-3">EMR</th>
                  ...
                  {sortedVariance.map((row) => {
                    ...
                    return (
                      <tr key={row.item} className="border-b border-white/5">
                        ...
                        <td
                          className={`py-2.5 tabular-nums font-semibold flex items-center gap-1.5 ${flagged ? "text-danger" : "text-mint"}`}
                        >
                          {flagged && <FiAlertTriangle size={12} />}{" "}
                          {v > 0 ? `+${v}` : v}
                        </td>
                      </tr>
                    );
              ...
              <p className="text-[11.5px] text-slate-500 mt-4">
                Physical vs EMR variance of 10+ units is flagged for
                investigation (possible miscount, mispick, or pilferage).
              </p>
            </GlowCard>
          </FadeIn>
        )}
        ...
        {tab === "receiving" && (
          <FadeIn>
            <GlowCard className="overflow-x-auto">
              <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
                <FiTruck className="text-mint" /> Receiving Discrepancy Checker
              </h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-white/10">
                    <th className="pb-2 pr-3">GRN</th>
                    <th className="pb-2 pr-3">Item</th>
                    <th className="pb-2 pr-3">Ordered</th>
                    <th className="pb-2 pr-3">Invoiced</th>
                ...
                <tbody>
                  {RECEIVING_RECORDS.map((r) => {
                    const mismatch =
                      r.ordered !== r.invoiced ||
                      r.invoiced !== r.delivered ||
                      r.ordered !== r.delivered;
                    return (
                      <tr key={r.id} className="border-b border-white/5">
                        <td className="py-2.5 pr-3 text-slate-500 font-mono text-xs">
                          {r.id}
                        ...
                        <td
                          className={`py-2.5 pr-3 tabular-nums ${r.invoiced !== r.ordered ? "text-warn font-semibold" : "text-slate-300"}`}
                        >
                          {r.invoiced}
                        </td>
                        <td
                          className={`py-2.5 pr-3 tabular-nums ${r.delivered !== r.ordered ? "text-danger font-semibold" : "text-slate-300"}`}
                        >
                          {r.delivered}
                        </td>
                        <td className="py-2.5">
                          <span
                            className={`px-2 py-0.5 rounded-full text-[11px] font-bold border ${mismatch ? "text-danger bg-danger/10 border-danger/30" : "text-mint bg-mint/10 border-mint/30"}`}
                          >
                            {mismatch ? "Discrepancy" : "Clean"}
                          </span>
                        </td>
                      </tr>
                    );
        ...
        {tab === "search" && (
          <FadeIn>
            <GlowCard className="mb-6">
              <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
                <FiSearch className="text-ai-cyan" /> Smart Inventory Search —{" "}
                {unit}
              </h3>
              <div className="flex flex-col sm:flex-row gap-2 mb-2">
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && liveMode && runLiveSearch()
                  }
                  placeholder={
                    liveMode
                      ? "e.g. Which items are low at the unit but well-stocked centrally?"
                      : "Search by name or risk level..."
                  }
              ...
              <p className="text-[11.5px] text-slate-500">
                {liveMode
                  ? "Ask Claude a natural-language question across this unit's and central store inventory."
                  : "Instant keyword filter over this location's formulary. Switch on Live AI Mode to ask natural-language questions."}
              </p>
            </GlowCard>

            {liveLoading && (
              <GlowCard className="mb-6">
                <LiveThinking label="Searching inventory with Phantom..." />
              </GlowCard>
            )}
            {liveError && (
              <GlowCard className="mb-6">
                <AIErrorNote message={liveError} onRetry={runLiveSearch} />
              </GlowCard>
            )}
            {liveMode && liveAnswer && (
              <FadeIn>
              ...
              <GlowCard className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-white/10">
                      <th className="pb-2 pr-3">Medicine</th>
                      <th className="pb-2 pr-3">Unit Stock</th>
                      <th className="pb-2 pr-3">Central Stock</th>
                      <th className="pb-2">Risk</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredInventory.map((i) => {
                      const central = CENTRAL_STOCK.find(
                        (c) => c.name === i.name,
                      );
                      return (
                        <tr key={i.name} className="border-b border-white/5">
                          ...
                          <td
                            className={`py-2.5 pr-3 tabular-nums ${central && central.qty < central.reorderPoint ? "text-warn font-semibold" : "text-slate-300"}`}
                          >
                            {central ? central.qty : "—"}
                          </td>
