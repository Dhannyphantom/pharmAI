...
import { PageHeader, GlowCard, SeverityPill, FadeIn, PrimaryButton, LiveThinking, AIErrorNote, LiveModeNote } from "@/components/ui";
...
export default function InteractionsPage() {
  ...
  const staticInteraction = drugA && drugB ? getInteraction(drugA, drugB) : null;
  const interaction = liveMode ? liveResult : staticInteraction;
  const lineColor = interaction ? SEVERITY_LINE_COLOR[interaction.severity] || "#94A3B8" : "#334155";

  function pick(drug) {
    if (liveMode) return;
    if (drugA === drug) { setDrugA(null); return; }
    if (drugB === drug) { setDrugB(null); return; }
    if (!drugA) setDrugA(drug);
    else if (!drugB) setDrugB(drug);
    else { setDrugA(drug); setDrugB(null); }
  }
  ...
  async function checkLive() {
    ...
    try {
      const json = await askAIJson(`Drug A: ${customA.trim()}\nDrug B: ${customB.trim()}`, { system: INTERACTION_SYSTEM_PROMPT, maxTokens: 400 });
      setLiveResult(json);
    } catch (e) {
  ...
  return (
    <>
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-6 py-10">
        ...
        {liveMode ? (
          <GlowCard className="mb-6">
            <div className="grid sm:grid-cols-2 gap-3 mb-3">
              <input
                type="text" value={customA} onChange={(e) => setCustomA(e.target.value)}
              ...
              />
              <input
                type="text" value={customB} onChange={(e) => setCustomB(e.target.value)}
                placeholder="Drug B, e.g. Isosorbide Mononitrate"
                className="bg-white/[0.04] border border-white/12 rounded-xl px-4 py-3 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-ai-cyan"
              />
            </div>
            <PrimaryButton onClick={checkLive} disabled={liveLoading || !customA.trim() || !customB.trim()} className="flex items-center gap-2">
        ...
        {liveMode && liveLoading && <GlowCard className="mb-5"><LiveThinking label="Checking interaction with Claude..." /></GlowCard>}
        {liveMode && liveError && <GlowCard className="mb-5"><AIErrorNote message={liveError} onRetry={checkLive} /></GlowCard>}

        <GlowCard className="relative min-h-[220px] flex items-center justify-center overflow-hidden">
          {!drugA && !drugB && <p className="text-slate-500 text-sm">Select two medications above to visualize their interaction.</p>}
          ...
          {(drugA || drugB) && (
            <div className="w-full flex items-center justify-between gap-6 relative py-8">
              ...
              <div className="flex-1 relative h-1 flex items-center">
                ...
                {(!drugA || !drugB || !interaction) && <div className="h-[2px] w-full bg-white/10 border-dashed" />}
                {interaction && (
        ...
        {interaction && (
          <FadeIn delay={0.15} className="mt-5">
            {liveMode && <LiveModeNote>Live AI Mode — this analysis was generated live by Claude, not scripted</LiveModeNote>}
            <GlowCard>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-white font-bold">{drugA} + {drugB}</h3>
                <SeverityPill level={interaction.severity} />
              </div>
              <p className="text-[13px] text-slate-300 mb-3 leading-relaxed">{interaction.mechanism}</p>
              <p className="text-[13px] text-ai-cyan">→ {interaction.recommendation}</p>
            </GlowCard>
          </FadeIn>
        )}
      </main>
  ...
  return (
    <div className="flex flex-col items-center gap-2 w-28 sm:w-36 shrink-0">
      <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-2xl flex items-center justify-center border ${
        name ? "bg-gradient-to-br from-hospital-blue/30 to-ai-violet/30 border-ai-cyan/30 glow-blue" : "border-white/10 bg-white/[0.02]"
      }`}>
        <span className="text-2xl">💊</span>
      </div>
      <span className="text-xs sm:text-sm text-center font-medium text-slate-200">{name || "—"}</span>
    </div>
  );
}
