...
import {
  ...
  PAYMENT_TRACKER,
  getProcedureById,
} from "@/lib/surgicalData";
import {
  THEATRE_REQUESTS as INITIAL_REQUESTS,
  STATUS_ORDER,
} from "@/lib/theatreData";
...
export default function TheatrePage() {
  ...
  const activeProcedure = activeRequest
    ? getProcedureById(activeRequest.procedureId)
    : null;
  ...
  function simulatedValidate() {
    ...
    const procedure = PROCEDURES.find(
      (p) => p.name.toLowerCase() === reqProcedure.trim().toLowerCase(),
    );
    const standardNames = procedure
      ? procedure.checklist.map((c) => c.item.toLowerCase())
      : [];
    const requestedNames = reqItems
      .split(/[,\n]/)
      .map((s) => s.trim())
      .filter(Boolean);
      ...
      ? requestedNames
          .filter(
            (r) =>
              !standardNames.some(
                (s) =>
                  s.includes(r.toLowerCase()) || r.toLowerCase().includes(s),
              ),
          )
          .map(
            (r) =>
              `"${r}" is not part of the standard checklist for ${procedure.name}`,
          )
      : [];
  ...
  function submitRequest() {
    ...
    setAuditLog((log) => [
      {
        id: `DOC-${1000 + log.length}`,
        department,
        procedure: reqProcedure,
        items: reqItems,
        time: "just now",
      },
      ...log,
  ...
  return (
    <>
              ...
              <div key={status}>
                ...
                <div className="space-y-3">
                      ...
                      <FadeIn key={r.id}>
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between text-[11px] text-slate-500">
                            ...
                            <span className="flex items-center gap-1 text-ai-cyan font-semibold">
                              View checklist <FiArrowRight size={11} />
                            </span>
                          </div>
                        </GlowCard>
                      </FadeIn>
                  ...
                  {requests.filter((r) => r.status === status).length === 0 && (
                    <div className="text-[12px] text-slate-600 border border-dashed border-white/10 rounded-xl p-4 text-center">
                      Empty
                    </div>
                  )}
                </div>
          ...
          <FadeIn>
            <GlowCard className="mb-6">
              <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
                <FiFileText className="text-ai-violet" /> New Digital Request
              </h3>
              <div className="grid sm:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    value={reqProcedure}
                    onChange={(e) => setReqProcedure(e.target.value)}
              ...
              <textarea
                ...
                rows={3}
                placeholder="e.g. Spinal anaesthesia set, Oxytocin 10 IU, Sterile gauze packs"
              ...
              <div className="flex gap-3">
                ...
                <GhostButton
                  ...
                  disabled={!liveResult?.readyToSubmit}
                  className="flex items-center gap-2"
                >
            ...
            {liveLoading && (
              <GlowCard className="mb-6">
                <LiveThinking label="Validating with Phantom..." />
              </GlowCard>
            )}
            {liveError && (
              <GlowCard className="mb-6">
                <AIErrorNote message={liveError} onRetry={liveValidate} />
              </GlowCard>
            )}
            ...
            {liveResult && (
              <FadeIn>
                ...
                <GlowCard
                  className={`mb-6 ${liveResult.readyToSubmit ? "border-mint/25" : "border-warn/25"}`}
                >
                  ...
                  {liveResult.missingFields?.length > 0 && (
                    <div className="mb-3">
                      <div className="text-[11px] uppercase tracking-wide text-warn mb-1.5">
                        Missing Fields
                      </div>
                  ...
                  {liveResult.mismatches?.length > 0 && (
                    <div>
                      <div className="text-[11px] uppercase tracking-wide text-danger mb-1.5">
                        Possible Mismatches
                      </div>
            ...
            <GlowCard>
              <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
                <FiFileText className="text-mint" /> Automated Documentation Log
              </h3>
              {auditLog.length === 0 ? (
                <p className="text-slate-500 text-sm">
                  Submitted requests are automatically logged here for audit
                  purposes.
                </p>
              ) : (
                <div className="space-y-2">
                  {auditLog.map((entry) => (
                    <div
                      key={entry.id}
                      className="p-3 rounded-xl bg-white/[0.02] border border-white/10 text-[12.5px]"
                    >
                      ...
                      <div className="text-slate-200">
                        {entry.department} — {entry.procedure}
                      </div>
          ...
          <FadeIn>
            <GlowCard>
              <h3 className="text-white font-bold text-sm mb-5 flex items-center gap-2">
                <FiDollarSign className="text-warn" /> NHIS / Payment Tracker
              </h3>
                ...
                {PAYMENT_TRACKER.map((row) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-4 gap-2 text-center text-[11px] text-slate-400 mb-2 tabular-nums">
                      ...
                      <div>
                        <div
                          className={`font-semibold text-sm ${row.outstanding > 0 ? "text-danger" : "text-slate-300"}`}
                        >
                          {row.outstanding}
                        </div>
                        Outstanding
                    ...
                    <LevelBar
                      percent={(row.paid / row.billed) * 100}
                      color={
                        row.outstanding > 0
                          ? "var(--color-warn)"
                          : "var(--color-mint)"
      ...
      <Modal
        ...
        eyebrow={
          activeRequest
            ? `${activeRequest.id} · ${activeRequest.department}`
            : ""
        }
          ...
          <div>
            {activeRequest.urgency === "Emergency" && (
              <div className="flex items-center gap-2 text-[12px] font-semibold text-danger bg-danger/10 border border-danger/25 rounded-lg px-3 py-2 mb-4">
                <FiAlertOctagon size={13} /> Emergency case — prioritize
                preparation
              </div>
            )}
            ...
            <div className="mb-5">
              ...
              <div className="space-y-1.5">
                {activeProcedure.checklist.map((c) => (
                  <div
                    key={c.item}
                    className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/[0.02] border border-white/10 text-sm"
                  >
            ...
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wide text-warn mb-1 flex items-center gap-1.5">
                <FiAlertTriangle size={13} /> Also Prepare For — Possible
                Complications
              </h4>
              <p className="text-[11.5px] text-slate-500 mb-2.5">
                Not routinely dispensed, but worth having ready given how often
                these arise for this procedure.
              </p>
              <div className="space-y-1.5">
                {activeProcedure.complicationItems.map((c) => (
                  <div
                    key={c.item}
                    className={`px-3 py-2.5 rounded-lg border text-sm ${c.common ? "bg-warn/5 border-warn/25" : "bg-white/[0.02] border-white/10"}`}
                  >
            ...
            {activeRequest.status !== "Issued" && (
              <div className="flex justify-end mt-6 pt-4 border-t border-white/10">
                <PrimaryButton
                  onClick={advanceActiveAndClose}
                  className="flex items-center gap-2"
                >
                  Advance to{" "}
                  {STATUS_ORDER[STATUS_ORDER.indexOf(activeRequest.status) + 1]}{" "}
