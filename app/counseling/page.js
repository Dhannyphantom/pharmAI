...
import {
  FiInfo, FiClock, FiAlertOctagon, FiHeart, FiPackage, FiEye, FiActivity, FiZap,
  FiUser, FiBookOpen, FiShield, FiLayers,
} from "react-icons/fi";
import NavBar from "@/components/NavBar";
import { PageHeader, GlowCard, FadeIn, LiveThinking, AIErrorNote, LiveModeNote, PrimaryButton, SegmentedTabs, BackButton } from "@/components/ui";
import { COUNSELING_DRUGS } from "@/lib/counselingData";
import { useApp } from "@/context/AppContext";
import { askAIJson } from "@/lib/aiClient";

const PATIENT_SECTIONS = [
  { key: "explanation", label: "What This Medicine Does", icon: FiInfo, single: true },
  { key: "howToTake", label: "How To Take It", icon: FiClock, single: true },
  { key: "commonSideEffects", label: "Common Side Effects", icon: FiActivity, list: true },
  { key: "seriousSideEffects", label: "Serious Side Effects — Seek Help", icon: FiAlertOctagon, list: true, danger: true },
  { key: "lifestyleAdvice", label: "Lifestyle Advice", icon: FiHeart, list: true },
  { key: "missedDose", label: "If You Miss A Dose", icon: FiClock, single: true },
...
const CLINICAL_FIELDS = [
  ...
  { key: "contraindications", label: "Contraindications", icon: FiShield, danger: true },
  { key: "keyInteractions", label: "Key Interactions", icon: FiShield, danger: true },
];
...
export default function CounselingPage() {
  ...
  async function generateLive(name) {
    ...
    setLiveCard(null);
    try {
      const json = await askAIJson(`Medication name: ${name.trim()}`, { system: COUNSELING_SYSTEM_PROMPT, maxTokens: 900 });
      setLiveCard(json);
    } catch (e) {
  ...
  const staticDrug = selected ? COUNSELING_DRUGS.find((x) => x.id === selected) : null;
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-6 py-10">
        ...
        {liveMode && (
          <div className="flex flex-col sm:flex-row gap-2 mb-6">
            ...
            />
            <PrimaryButton onClick={() => generateLive(customDrug)} disabled={liveLoading || !customDrug.trim()} className="flex items-center gap-2 justify-center">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {COUNSELING_DRUGS.map((d) => (
            <button
              key={d.id}
              onClick={() => (liveMode ? generateLive(d.name) : setSelected(d.id === selected ? null : d.id))}
        ...
        {liveMode && liveLoading && <GlowCard><LiveThinking label="Generating counselling card with Claude..." /></GlowCard>}
        {liveMode && liveError && <GlowCard><AIErrorNote message={liveError} /></GlowCard>}
        ...
        <AnimatePresence mode="wait">
          {displayed && (
            ...
            >
              {liveMode ? (
                <LiveModeNote>Live AI Mode — this card was generated live by Claude, not scripted</LiveModeNote>
              ) : (
                <div className="mb-4 inline-flex items-center gap-2 text-[11px] font-semibold tracking-wide uppercase text-ai-cyan bg-ai-cyan/10 border border-ai-cyan/25 rounded-full px-3 py-1.5">
                  ✨ AI Generated Draft — Must Be Reviewed By a Pharmacist Before Use
                </div>
              )}
              <h2 className="text-2xl font-bold text-white mb-5">{displayed.name}</h2>

              <SegmentedTabs tabs={VIEW_TABS} active={view} onChange={setView} />
              ...
              {view === "patient" && (
                <div className="grid sm:grid-cols-2 gap-4">
                  {PATIENT_SECTIONS.map((s, i) => {
                    ...
                    return (
                      <FadeIn key={s.key} delay={i * 0.06} className={s.single && s.key === "explanation" ? "sm:col-span-2" : ""}>
                        <GlowCard className={s.danger ? "border-danger/30 bg-danger/5" : ""}>
                          <div className="flex items-center gap-2 mb-2">
                            <Icon className={s.danger ? "text-danger" : "text-ai-cyan"} size={16} />
                            <h3 className={`text-sm font-bold ${s.danger ? "text-danger" : "text-white"}`}>{s.label}</h3>
                          </div>
                          {Array.isArray(content) ? (
                            ...
                            <p className="text-[13px] text-slate-300 leading-relaxed">{content}</p>
                          )}
                        </GlowCard>
                      </FadeIn>
                    );
              ...
              {view === "clinical" && (
                <div className="grid sm:grid-cols-2 gap-4">
                  {!displayed.clinical ? (
                    <div className="sm:col-span-2 text-slate-500 text-sm py-8 text-center">No clinical reference available for this entry.</div>
                  ) : (
                    CLINICAL_FIELDS.map((f, i) => {
                      ...
                      return (
                        <FadeIn key={f.key} delay={i * 0.06} className={f.key === "drugClass" ? "sm:col-span-2" : ""}>
                          <GlowCard className={f.danger ? "border-danger/30 bg-danger/5" : ""}>
                            <div className="flex items-center gap-2 mb-2">
                              <Icon className={f.danger ? "text-danger" : "text-ai-violet"} size={16} />
                              <h3 className={`text-sm font-bold ${f.danger ? "text-danger" : "text-white"}`}>{f.label}</h3>
                            </div>
                            <p className="text-[13px] text-slate-300 leading-relaxed">{content}</p>
                          </GlowCard>
                        </FadeIn>
        ...
        {!displayed && !liveLoading && !liveError && (
          <div className="text-center text-slate-500 text-sm py-16">
            {liveMode ? "Type a medication above, or pick a quick preset, to generate its counselling card live." : "Select a medication above to generate its counselling card."}
          </div>
        )}
      </main>
    </>
