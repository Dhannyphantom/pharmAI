...
import { PageHeader, GlowCard, FadeIn, PrimaryButton, GhostButton, SeverityPill, BackButton, LiveThinking, AIErrorNote, LiveModeNote } from "@/components/ui";
...
export default function DuplicateCheckPage() {
  const [currentMedsText, setCurrentMedsText] = useState(DEFAULT_SCENARIO.currentMedsText);
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-6 py-10">
        ...
        <GlowCard className="mb-6">
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-wide text-slate-400 mb-2">Current Medications (one per line)</label>
              <textarea
            ...
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-wide text-slate-400 mb-2">New Prescription</label>
              <input
                type="text"
                value={newDrug}
              ...
              <p className="text-[11px] text-slate-500 mt-2 leading-relaxed">
                {liveMode ? "Claude will reason about drug class overlap." : "Checked against a local therapeutic-class reference of ~45 common drugs."}
              </p>
            </div>
          </div>
          <div className="flex gap-3">
            <PrimaryButton onClick={check} disabled={liveLoading || !currentMedsText.trim() || !newDrug.trim()} className="flex items-center gap-2">
              {liveMode && <FiZap size={14} />} {liveMode ? "Check With Live AI" : "Check for Duplicates"}
            </PrimaryButton>
            <GhostButton onClick={() => { setCurrentMedsText(DEFAULT_SCENARIO.currentMedsText); setNewDrug(DEFAULT_SCENARIO.newDrug); setResult(null); }}>
              Reset Example
            </GhostButton>
          </div>
        </GlowCard>

        {liveLoading && <GlowCard className="mb-6"><LiveThinking label="Comparing therapy with Claude..." /></GlowCard>}
        {liveError && <GlowCard className="mb-6"><AIErrorNote message={liveError} onRetry={runLive} /></GlowCard>}

        {result && (
          <FadeIn>
            {liveMode && <LiveModeNote>Live AI Mode — generated by Claude, not scripted</LiveModeNote>}
            <GlowCard className={result.duplicates.length ? "border-warn/25" : "border-mint/25"}>
              <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                <div>
                  <div className="text-[11px] uppercase tracking-wide text-slate-500 mb-1">New Prescription</div>
                  <div className="text-white font-semibold">{result.newDrug} <span className="text-slate-400 font-normal text-sm">— {result.newClass}</span></div>
                </div>
              ...
              {result.duplicates.length === 0 ? (
                <div className="flex items-center gap-2.5 p-4 rounded-xl bg-mint/5 border border-mint/20 text-mint text-sm">
                  <FiCheckCircle /> No duplicate or overlapping therapy detected against the current chart.
                </div>
              ) : (
                <div className="space-y-2.5">
                  {result.duplicates.map((d, i) => (
                    <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-warn/5 border border-warn/20">
                      <FiAlertTriangle className="text-warn shrink-0 mt-0.5" size={15} />
                      <div>
                        <div className="text-sm font-semibold text-white">{d.drug} <span className="text-slate-400 font-normal">— {d.drugClass}</span></div>
                        <div className="text-[12.5px] text-slate-300 mt-0.5">{d.reason}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {!result && !liveLoading && (
          <div className="flex items-center gap-2 text-slate-500 text-sm py-6">
            <FiCopy className="shrink-0" /> Enter a chart above and run the check to see it in action.
          </div>
        )}
      </main>
    </>
