...
import { FiDollarSign, FiAlertTriangle, FiBarChart2, FiZap, FiClock } from "react-icons/fi";
import NavBar from "@/components/NavBar";
import { PageHeader, GlowCard, FadeIn, StaggerList, SegmentedTabs, PrimaryButton, SeverityPill, BackButton, LiveThinking, AIErrorNote, LiveModeNote } from "@/components/ui";
import { BILLING_RECORDS, ERROR_LOG, QUEUE_DATA, getPeakHour } from "@/lib/nhisOpsData";
...
export default function NhisOpsPage() {
  ...
  const mismatchCount = BILLING_RECORDS.filter((r) => r.status === "Mismatch").length;
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-6 py-10">
          ...
          <FadeIn>
            <GlowCard className="mb-6 overflow-x-auto">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                <h3 className="text-white font-bold text-sm flex items-center gap-2"><FiDollarSign className="text-ai-cyan" /> Billing Verification</h3>
                {mismatchCount > 0 && (
                ...
                <tbody>
                  {BILLING_RECORDS.map((r) => (
                    <tr key={r.id} className="border-b border-white/5">
                      <td className="py-2.5 pr-3 text-slate-200 font-medium">{r.patient}</td>
                      <td className="py-2.5 pr-3 text-slate-300">{r.drug}</td>
                      <td className="py-2.5 pr-3 text-slate-300 tabular-nums">{r.prescribedQty}</td>
                      <td className={`py-2.5 pr-3 tabular-nums ${r.billedQty !== r.prescribedQty ? "text-danger font-semibold" : "text-slate-300"}`}>{r.billedQty}</td>
                      <td className="py-2.5 pr-3 text-slate-500 text-xs">{r.receiptNo}</td>
                      <td className="py-2.5">
                        <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold border ${BILLING_STATUS_STYLE[r.status]}`}>{r.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {tab === "errors" && (
          <FadeIn>
            <GlowCard className="mb-6">
              <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2"><FiAlertTriangle className="text-warn" /> Prescription Error Dashboard</h3>
              <StaggerList
                items={ERROR_LOG}
                renderItem={(e) => (
                  <div className="flex items-start justify-between gap-3 p-4 rounded-xl bg-white/[0.02] border border-white/10">
                    <div>
                      ...
                      <p className="text-[12.5px] text-slate-300">{e.description}</p>
                      <p className="text-[11px] text-slate-500 mt-1">{e.drug} · {e.time}</p>
                    </div>
                  </div>
                )}
              />
        ...
        {tab === "queue" && (
          <FadeIn>
            <GlowCard className="mb-6">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                <h3 className="text-white font-bold text-sm flex items-center gap-2"><FiBarChart2 className="text-ai-violet" /> Queue Analytics — Today</h3>
                <span className="flex items-center gap-1.5 text-[11px] font-semibold text-ai-violet bg-ai-violet/10 border border-ai-violet/25 rounded-full px-3 py-1">
                  <FiClock size={11} /> Peak: {peak.hour} ({peak.patients} patients)
                </span>
              </div>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={QUEUE_DATA}>
                    ...
                    <Bar dataKey="patients" fill="#7C5CFF" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </GlowCard>
        ...
        {liveMode && (
          <GlowCard>
            <PrimaryButton onClick={briefOps} disabled={liveLoading} className="flex items-center gap-2 mb-3">
              <FiZap size={14} /> Ask Live AI for Today&apos;s Operations Briefing
            </PrimaryButton>
            {liveLoading && <LiveThinking label="Analyzing operations with Claude..." />}
            {liveError && <AIErrorNote message={liveError} onRetry={briefOps} />}
            {liveText && (
              <FadeIn>
