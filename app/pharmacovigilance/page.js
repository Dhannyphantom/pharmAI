        ...
        <div className="grid lg:grid-cols-[1fr_1fr] gap-6">
          <GlowCard>
            <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
              <FiActivity className="text-ai-cyan" /> Incoming Reports
              (Simulated Feed)
            </h3>
            <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
              {PHARMACOVIGILANCE_REPORTS.map((r) => (
                <div
                  key={r.id}
                  className="flex items-center justify-between p-3 rounded-lg bg-white/[0.02] border border-white/10 text-[13px]"
                >
                  <div className="flex items-center gap-2.5">
                    <span
                      className={`w-2 h-2 rounded-full ${SEVERITY_DOT[r.severity]}`}
                    ></span>
                    <div>
          ...
          <GlowCard>
            <h3 className="text-white font-bold text-sm mb-4 flex items-center gap-2">
              <FiAlertTriangle className="text-warn" /> AI Signal Detection
            </h3>
            {!signalRevealed ? (
              <div className="flex flex-col items-center justify-center py-10 gap-4">
                <p className="text-sm text-slate-400 text-center max-w-xs">
                  {liveMode
                    ? "Have Claude analyze the report feed for clustering signals."
                    : "AI has been clustering the report stream in the background. Reveal what it found."}
                </p>
                <button
                ...
                >
                  ...
                  {liveMode ? "Analyze Live With AI" : "Reveal Detected Signal"}
                </button>
              </div>
            ) : liveMode && liveLoading ? (
              <LiveThinking label="Clustering reports with Phantom..." />
            ) : liveMode && liveError ? (
              ...
              <FadeIn>
                ...
                <div className="p-4 rounded-xl bg-danger/10 border border-danger/25">
                  <div className="text-sm font-bold text-danger mb-1">
                    {signal.reportCount} reports · {signal.drug}{" "}
                    {!liveMode && `· ${SIGNAL_CLUSTER.timeWindow}`}
                  </div>
                  <div className="text-[13px] text-slate-200 font-medium mb-2">
                    {signal.reactionCluster}
                  </div>
